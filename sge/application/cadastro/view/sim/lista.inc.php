<?php
    ...
    $sql = "SELECT s.*, c.cid_nome, c.est_uf FROM sim AS s INNER JOIN cidade AS c ON (c.cid_cod = s.cid_cod) WHERE sim_situacao = 1 ";
    $ati = $data->find('dynamic', $sql);

    $sql = "SELECT s.*, c.cid_nome FROM sim AS s INNER JOIN cidade AS c ON (c.cid_cod = s.cid_cod) WHERE sim_situacao = 0";
                                ...
                                <table class="table table-striped table-bordered table-hover dataTables-example">
                                    ...
                                    <tbody>
                                        ...
                                        for ($i = 0; $i < count($ati); $i++) {
                                            echo '
                                                <tr>
                                                    <td>' . str_pad($ati[$i]['sim_cod'], 4, '0', STR_PAD_LEFT) . '</td>
                                                    <td>' . $ati[$i]['sim_responsavel'] . '</td>
                                                    <td>' . $ati[$i]['sim_email'] . '</td>
                                                    <td>' . $ati[$i]['sim_telefone'] . '</td>
                                                    <td>' . $ati[$i]['cid_nome'].' - '.$ati[$i]['est_uf'] . '</td>
                                                    <td>
                                                        <a href="#" onclick="nextPage(\'?module=cadastro&acao=edita_sim\', ' . $ati[$i]['sim_cod'] . ')">
                                                            <span class="fa-stack">
                                                                <i class="fa fa-square fa-stack-2x"></i>
                                                                <i class="fa fa-pencil fa-stack-1x fa-inverse"></i>
                                                            </span>
                                                        </a>
                                                        <a href="#" onClick=\'inativar("' . $ati[$i]['sim_cod'] . '", "' . $ati[$i]['sim_cod'] . '");\' title="Inativar SIM" style="text-decoration:none;">
                                                            <span class="fa-stack">
                                                                <i class="fa fa-square fa-stack-2x"></i>
                                    ...
                                    <tbody>
                                        ...
                                        for ($i = 0; $i < count($ina); $i++) {
                                            echo '
                                                <tr>
                                                    <td>' . $ina[$i]['sim_cod'] . '</td>
                                                    <td>' . $ina[$i]['sim_responsavel'] . '</td>
                                                    <td>' . $ina[$i]['sim_email'] . '</td>
                                                    <td>' . $ina[$i]['sim_telefone'] . '</td>
                                                    <td>' . $ina[$i]['cid_nome'].' - '.$ina[$i]['est_uf'] . '</td>
                                                    <td>
                                                        <a href="#" onClick=\'ativar("' . $ina[$i]['sim_cod'] . '", "' . $ina[$i]['sim_cod'] . '");\' title="Reativar SIM" style="text-decoration:none;">
                                                            <span class="fa-stack">
                                                                <i class="fa fa-square fa-stack-2x"></i>
